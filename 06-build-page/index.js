...
const copyFolder = async (src, target) => {
    ...
    await copyAssets(src, newFolder);
    };
    copyFolder(src, target);


const copyAssets = async (src, target) => {
  await fs.mkdir(target, {recursive: true}, () => {});
  fs.readdir(src, {withFileTypes: true}, (err, files) => {
    if (err) throw err;
    files.forEach((file) => {
      let srcPth = path.join(src, file.name);
      let targetPth = path.join(target, file.name);
      if(file.isDirectory()) {
        copyAssets(srcPth, targetPth);
      } else {
        fs.copyFile(srcPth, targetPth,() => {});
      }
    });
  });
};
